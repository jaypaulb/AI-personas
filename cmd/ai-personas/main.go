	...
	for {
		...
		case trig := <-triggers:
			...
			if trig.Widget.Type == "Image" && imageTitle == "bac_complete" && !imagePatched {
				...
				if err != nil {
					logutil.Errorf("[error] Failed to fetch widgets: %v\n", err)
					continue
				}
				...
				if personasAnchor == nil {
					logutil.Errorf("Note - Personas anchor not found Aborting\n")
					continue
				}
				...
				for i, p := range personas {
					...
					if err != nil {
						logutil.Warnf("[warn] Failed to create persona note: %v\n", err)
					} else {
						noteWidgetID, _ := noteWidget["id"].(string)
						logutil.Infof("[action] Persona note created: %s (ID: %s)", p.Name, noteWidgetID)
					}
					// Start image generation/upload in a goroutine
					imgWg.Add(1)
					go func(p gemini.Persona, x, imgY, imgW, imgHpx float64) {
						defer imgWg.Done()
						logutil.Debugf("[debug] Calling OpenAI DALL·E for persona: %s", p.Name)
						imgBytes, err := gemini.GeneratePersonaImageOpenAI(p)
						logutil.Debugf("[debug] OpenAI DALL·E call returned for persona: %s, err: %v", p.Name, err)
						imgPath := ""
						if err != nil {
							logutil.Warnf("[warn] Persona image not generated: %v\n", err)
							return
						}
						tmpfile, err := os.CreateTemp("", "persona_*.png")
						if err != nil {
							logutil.Warnf("[warn] Could not create temp file for persona image: %v\n", err)
							return
						}
						imgPath = tmpfile.Name()
						if _, err := tmpfile.Write(imgBytes); err != nil {
							logutil.Warnf("[warn] Could not write persona image to temp file: %v\n", err)
							tmpfile.Close()
							os.Remove(imgPath)
							return
						}
						...
						if err != nil {
							logutil.Warnf("[warn] Failed to upload persona image: %v\n", err)
						} else {
							imgWidgetID, _ := imgWidget["id"].(string)
							logutil.Infof("[action] Persona image uploaded: %s (ID: %s)", p.Name, imgWidgetID)
						}
						os.Remove(imgPath)
			...
			if trig.Widget.Type == "Note" && trig.Widget.Title == "New_AI_Question" && !notePatched {
				...
				respJSON, _ := json.MarshalIndent(resp, "", "  ")
				logutil.Debugf("[action] UpdateNote (to _Monitoring) response:\n%s\nerr: %v\n", string(respJSON), err)
				notePatched = true
				...
				go func(noteID string, noteTextExtracted *string, imageID *string) {
					...
					for {
						...
						default:
							...
							logutil.Debugf("[note-monitor] Raw event: %s\n", string(line))
							var raw map[string]interface{}
							if err := json.Unmarshal(line, &raw); err != nil {
								logutil.Debugf("[note-monitor] Skipping malformed line: %s\n", string(line))
								continue
							}
							text, _ := raw["text"].(string)
							logutil.Debugf("[note-monitor] Note event received: text=%q\n", text)
							if !patched {
								patched = true
								time.Sleep(2 * time.Second)
								...
								logutil.Debugf("[note-monitor] Patching note text. Before: %q, After: %q\n", text, newText)
								resp, err := client.UpdateNote(noteID, updateText)
								respJSON, _ := json.MarshalIndent(resp, "", "  ")
								logutil.Debugf("[note-monitor] UpdateNote (add '?') response:\n%s\nerr: %v\n", string(respJSON), err)
								fetched, fetchErr := client.GetNote(noteID, false)
								if fetchErr != nil {
									logutil.Errorf("[note-monitor] Error fetching note after patch: %v\n", fetchErr)
								} else {
									fetchedText, _ := fetched["text"].(string)
									logutil.Debugf("[note-monitor] Note text after patch (fetched): %q\n", fetchedText)
								}
							}
							if strings.HasSuffix(strings.TrimSpace(text), "?") {
								logutil.Infof("[note-monitor] Note text now ends with '?', extracting.\n")
